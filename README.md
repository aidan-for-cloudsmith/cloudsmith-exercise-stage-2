...
With more time, I would add tenant isolation to the the api with authentication (probably jwt). idempotent ingestion, stronger schema validation, scheduled retention cleanup, pagination, and more integration tests around malformed data and cross-tenant access. I'd also look at the performance of the API (I assume the issue is that minio isn't optimised for performance - it's just a stub), switching to parquet files should give a bit boost here.